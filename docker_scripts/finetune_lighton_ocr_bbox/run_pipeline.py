...
print("🚀 Starting LightOnOCR-2-1B-bbox Fine-Tuning Pipeline...", flush=True)
...
if model_out.exists() and (model_out / "config.json").exists():
    ...
    run_step("2️⃣  Starting Fine-Tuning (LoRA + bbox)...", "train_lighton_bbox.py")
...
try:
    ...
    print(f"📦 Uploading merged weights to {repo_id}...", flush=True)
    api.upload_folder(
...
print("\n🎉 LightOn bbox Pipeline Finished!", flush=True)
terminate_runpod()
