...
export default function LocalOcrStatusIndicator() {
    ...
    const summaryLabel = isCheckingLocalConnection
        ...
        : connectionStatus === 'reconnecting'
            ...
            : connectionStatus === 'offline'
                ? "Hors ligne"
                : downloadActive
                    ? `Telechargement${downloadPercent !== null ? ` ${downloadPercent}%` : ""}`
    ...
    const tone = connectionStatus === 'offline'
        ...
        : connectionStatus === 'reconnecting' || connectionStatus === 'degraded'
            ? 'amber'
            : downloadActive || isLoadingLocalModel || localModelStatus?.loading
                ? 'blue'
                : localModelStatus?.ready
                    ? 'emerald'
                    : 'slate';
    ...
    const canDownload = !localModelStatus?.installed && !downloadActive;
    const canLoad = localModelStatus?.installed && !localModelStatus?.ready && !isLoadingLocalModel && !downloadActive;
            ...
            <div className="pointer-events-none absolute left-0 top-full hidden w-[360px] pt-2 group-hover:block group-focus-within:block group-hover:pointer-events-auto group-focus-within:pointer-events-auto">
                <div className="rounded-lg border border-slate-200 bg-white p-3 text-slate-900 shadow-xl">
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                        <DetailRow label="Modele" value={localLoaded ? "Charge" : localModelStatus?.installed ? "Installe" : "Absent"} valueClassName={localLoaded ? "text-emerald-700" : ""} />
                        <DetailRow label="Pret" value={localModelStatus?.ready ? "Oui" : "Non"} valueClassName={localModelStatus?.ready ? "text-emerald-700" : ""} />
                        ...
                        <DetailRow label="Dtype" value={localModelStatus?.dtype || "-"} />
                    ...
                    {(downloadActive || localDownloadState?.downloaded_bytes || localDownloadState?.error) && (
                        <div className="mt-3 border-t border-slate-100 pt-2">
                            <div className="mb-1 flex items-center justify-between text-[11px] font-bold text-slate-600">
                                <span>Telechargement modele</span>
                                <span>{downloadPercent !== null ? `${downloadPercent}%` : downloadBytesLabel}</span>
                            </div>
                            <div className="h-1.5 w-full overflow-hidden rounded-full bg-slate-200">
                                <div
                                    className={cn("h-full rounded-full transition-all", localDownloadState?.error ? "bg-amber-500" : "bg-emerald-500", downloadPercent === null && downloadActive && "animate-pulse")}
                                    style={{ width: `${downloadPercent ?? (downloadActive ? 35 : 100)}%` }}
                                />
                            </div>
