...
describe('postOcrImage', () => {
    ...
    it.each(['/api/ocr', '/api/local_lighton', '/api/poneglyph_one_shot'])('rejects anonymous requests even with the legacy bypass option: %s', async (endpoint) => {
        supabase.auth.getSession.mockResolvedValue({ data: { session: null }, error: null });
        const fetchImpl = vi.fn();
        await expect(postOcrImage(endpoint, new Blob(['jpeg'], { type: 'image/jpeg' }), {
            allowAnonymous: true, fetchImpl,
        })).rejects.toMatchObject({ code: 'OCR_AUTH_REQUIRED' });
        expect(fetchImpl).not.toHaveBeenCalled();
    });

    it('uses Bearer auth for full-page OCR when a session exists', async () => {
        supabase.auth.getSession.mockResolvedValue({
